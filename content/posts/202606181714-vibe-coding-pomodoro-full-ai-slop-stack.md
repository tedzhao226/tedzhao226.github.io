...
- The one I didn't expect: Cmux lets you orchestrate a Claude Code instance without paying for it twice. The usual way to automate Claude Code is to call it programmatically with `claude -p` (or the SDK), but that path runs as metered API usage, billed separately from the Claude Code subscription you already pay for. Cmux skips it. It drives your live, interactive Claude Code session by sending keystrokes into its terminal panel the way tmux send-keys does, so you are automating the same subscription session you are already in, with no extra API tokens. And because it is only driving a panel, the same trick invokes any agent that lives in a terminal, even one with no CLI or API at all. If it runs in a window, Cmux can run it.
